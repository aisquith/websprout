...
const NAV_CONTRAST_SCRIPT = '<scr'+'ipt id="_wsNavContrast">(function(){function lum(c){var m=(c||"").match(/rgba?\\(([^)]+)\\)/);if(!m)return -1;var p=m[1].split(",").map(function(x){return parseFloat(x);});var a=p.length>3?p[3]:1;if(a<0.05)return -1;function ch(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4);}return 0.2126*ch(p[0])+0.7152*ch(p[1])+0.0722*ch(p[2]);}function bgOf(el){while(el&&el!==document.body){var sy=getComputedStyle(el);var mm=(sy.backgroundColor||"").match(/rgba?\\(([^)]+)\\)/);if(mm){var aa=mm[1].split(",")[3];if(aa===undefined||parseFloat(aa)>0.5)return sy.backgroundColor;}el=el.parentElement;}return getComputedStyle(document.body).backgroundColor||"rgb(255,255,255)";}function fix(){var nav=document.querySelector("header nav")||document.querySelector("header")||document.querySelector("nav");if(!nav)return;var navBg=bgOf(nav);var navL=lum(navBg);if(navL<0)return;var wantDark=navL>0.55;var targets=nav.querySelectorAll("a,button,span,strong,em,li,h1,h2,h3,h4,h5,h6");for(var i=0;i<targets.length;i++){var t=targets[i];var isBtn=t.tagName==="BUTTON"||(t.tagName==="A"&&t.className&&/btn|button|cta/i.test(t.className));if(isBtn)continue;var col=getComputedStyle(t).color;var cl=lum(col);if(cl<0)continue;var contrast=Math.abs(cl-navL);if(contrast<0.15){t.style.setProperty("color",wantDark?"#111":"#fff","important");}}}fix();setTimeout(fix,600);window.addEventListener("scroll",function(){fix();},{passive:true});})();</scr'+'ipt>';
...
<script>window._wsBuild="2026-06-10-r232";console.log("%c[Websprout] build 2026-06-10-r235 — nav contrast: elevated prompt rule + deterministic runtime fixer","color:#4ade80;font-weight:700")</script>
