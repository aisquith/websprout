...
const NAV_CONTRAST_SCRIPT = '<scr'+'ipt id="_wsNavContrast">(function(){function lum(c){var m=(c||"").match(/rgba?\\(([^)]+)\\)/);if(!m)return -1;var p=m[1].split(",").map(function(x){return parseFloat(x);});var a=p.length>3?p[3]:1;if(a<0.05)return -1;function ch(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4);}return 0.2126*ch(p[0])+0.7152*ch(p[1])+0.0722*ch(p[2]);}function opaqueBgOf(el){while(el&&el!==document.documentElement){var sy=getComputedStyle(el);var mm=(sy.backgroundColor||"").match(/rgba?\\(([^)]+)\\)/);if(mm){var aa=mm[1].split(",")[3];if(aa===undefined||parseFloat(aa)>0.5)return sy.backgroundColor;}el=el.parentElement;}return getComputedStyle(document.body).backgroundColor||"rgb(255,255,255)";}function behindNav(nav){var r=nav.getBoundingClientRect();var x=r.left+r.width/2;var y=r.top+r.height/2;var prevPE=nav.style.pointerEvents;var prevVis=nav.style.visibility;nav.style.pointerEvents="none";nav.style.visibility="hidden";var el=document.elementFromPoint(x,y);nav.style.pointerEvents=prevPE;nav.style.visibility=prevVis;return el||document.body;}function fix(){var nav=document.querySelector("header nav")||document.querySelector("header")||document.querySelector("nav");if(!nav)return;var sy=getComputedStyle(nav);var navBgMatch=(sy.backgroundColor||"").match(/rgba?\\(([^)]+)\\)/);var navAlpha=navBgMatch?(navBgMatch[1].split(",")[3]===undefined?1:parseFloat(navBgMatch[1].split(",")[3])):0;var effectiveBg;if(navAlpha>0.5){effectiveBg=sy.backgroundColor;}else{var under=behindNav(nav);effectiveBg=opaqueBgOf(under);}var bgL=lum(effectiveBg);if(bgL<0)return;var wantDark=bgL>0.55;var targets=nav.querySelectorAll("a,button,span,strong,em,li,div,h1,h2,h3,h4,h5,h6,p");for(var i=0;i<targets.length;i++){var t=targets[i];if(t.children.length&&t.textContent.trim()===(t.children[0].textContent||"").trim())continue;var isBtn=t.tagName==="BUTTON"||(t.tagName==="A"&&t.className&&/btn|button|cta/i.test(t.className));if(isBtn)continue;var col=getComputedStyle(t).color;var cl=lum(col);if(cl<0)continue;var contrast=Math.abs(cl-bgL);if(contrast<0.25){t.style.setProperty("color",wantDark?"#111":"#fff","important");}}}fix();setTimeout(fix,400);setTimeout(fix,1200);window.addEventListener("scroll",function(){fix();},{passive:true});window.addEventListener("resize",function(){fix();},{passive:true});})();</scr'+'ipt>';
...
const NAV_DEDUPE_SCRIPT = '<scr'+'ipt id="_wsNavDedupe">(function(){function txt(a){return (a.textContent||"").trim().toLowerCase();}function vis(el){return !!el&&!!el.offsetParent&&el.getBoundingClientRect().width>0;}function links(g){var a=g.querySelectorAll("a"),r=[];for(var i=0;i<a.length;i++){if(txt(a[i]))r.push(a[i]);}return r;}function orient(ls){var t=[],l=[];for(var i=0;i<ls.length;i++){var r=ls[i].getBoundingClientRect();t.push(r.top);l.push(r.left);}var cs=Math.max.apply(null,t)-Math.min.apply(null,t),rs=Math.max.apply(null,l)-Math.min.apply(null,l);return cs>rs?"v":"h";}function tset(ls){var o={};for(var i=0;i<ls.length;i++)o[txt(ls[i])]=1;return o;}function jac(a,b){var u={},n=0,c=0;for(var k in a){u[k]=1;if(b[k])c++;}for(var k2 in b)u[k2]=1;for(var k3 in u)n++;return n?c/n:0;}function bullets(){var q=document.querySelectorAll("header ul,header ol,nav ul,nav ol,ul[class*=nav],ul[class*=menu]");for(var i=0;i<q.length;i++)q[i].style.setProperty("list-style","none","important");}function restore(){var h=document.querySelectorAll("[data-wshid]");for(var i=0;i<h.length;i++){h[i].style.removeProperty("display");h[i].removeAttribute("data-wshid");}}function run(){bullets();if((window.innerWidth||0)<900){restore();return;}var burg=document.querySelectorAll("[class*=burger],[class*=Burger],[class*=hamburger],[class*=Hamburger]");for(var b=0;b<burg.length;b++){if(vis(burg[b])){burg[b].style.setProperty("display","none","important");burg[b].setAttribute("data-wshid","1");}}var raw=document.querySelectorAll("nav,ul"),g=[];for(var i=0;i<raw.length;i++){var ls=links(raw[i]);if(ls.length>=2&&vis(raw[i]))g.push({el:raw[i],o:orient(ls),t:tset(ls)});}var hasH=false;for(var h2=0;h2<g.length;h2++)if(g[h2].o==="h")hasH=true;if(!hasH)return;for(var v=0;v<g.length;v++){if(g[v].o!=="v"||g[v].el.tagName!=="UL")continue;for(var w=0;w<g.length;w++){if(g[w].o!=="h")continue;if(g[v].el!==g[w].el&&jac(g[v].t,g[w].t)>=0.5&&!g[v].el.contains(g[w].el)&&!g[w].el.contains(g[v].el)){g[v].el.style.setProperty("display","none","important");g[v].el.setAttribute("data-wshid","1");break;}}}}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",run);}else{run();}setTimeout(run,400);setTimeout(run,1200);window.addEventListener("resize",run,{passive:true});})();</scr'+'ipt>';
function withReveal(html){
  if(!html||html.indexOf('_wsReveal')>-1)return html;
  var bi=html.lastIndexOf('</body>');
  var _both = NAV_CONTRAST_SCRIPT + CTA_EDGE_SCRIPT + NAV_DEDUPE_SCRIPT + REVEAL_SCRIPT;
...
<meta name="ws-build" content="2026-06-10-r245">
<script>window._wsBuild="2026-06-10-r245";console.log("%c[Websprout] build 2026-06-10-r245 — nav-dedupe enforcer: mobile menu never leaks onto desktop","color:#4ade80;font-weight:700")</script>
...
const BUILD_ID = '2026-06-10-r245';
