...
const NAV_CONTRAST_SCRIPT = '<scr'+'ipt id="_wsNavContrast">(function(){function prgb(c){c=c||"";var i=c.indexOf("(");if(i<0)return null;var j=c.indexOf(")");var a=c.slice(i+1,j).split(",");var o=[];for(var k=0;k<a.length;k++)o.push(parseFloat(a[k]));return o.length>=3?o:null;}function al(pp){return pp.length>3?pp[3]:1;}function lum(pp){if(!pp||al(pp)<0.05)return -1;function ch(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4);}return 0.2126*ch(pp[0])+0.7152*ch(pp[1])+0.0722*ch(pp[2]);}function gradLum(bi){var i=0,sum=0,n=0;while(true){var a=bi.indexOf("rgb",i);if(a<0)break;var b=bi.indexOf(")",a);if(b<0)break;var pp=prgb(bi.slice(a,b+1));if(pp){var l=lum(pp);if(l>=0){sum+=l;n++;}}i=b+1;}return n?sum/n:-1;}function bgLumOf(el){while(el){var st=getComputedStyle(el);var pp=prgb(st.backgroundColor);if(pp&&al(pp)>0.5){var l=lum(pp);if(l>=0)return l;}var bi=st.backgroundImage||"";if(bi.indexOf("gradient")>-1){var gl=gradLum(bi);if(gl>=0)return gl;}if(el===document.documentElement)break;el=el.parentElement;}return -1;}function bgBehind(el){try{var r=el.getBoundingClientRect();if(r.width<1||r.height<1)return -1;var x=r.left+r.width/2,y=r.top+r.height/2;var vs=el.style.visibility,pe=el.style.pointerEvents;el.style.visibility="hidden";el.style.pointerEvents="none";var b=document.elementFromPoint(x,y);el.style.visibility=vs;el.style.pointerEvents=pe;return b?bgLumOf(b):-1;}catch(e){return -1;}}function fix(){var root=document.querySelector("header")||document.querySelector("nav");if(!root)return;var targets=root.querySelectorAll("a,button,span,strong,em,li,div,h1,h2,h3,h4,h5,h6,p,b");for(var i=0;i<targets.length;i++){var t=targets[i];if(!t.textContent||!t.textContent.trim())continue;if(t.children.length&&t.textContent.trim()===(t.children[0].textContent||"").trim())continue;var isBtn=t.tagName==="BUTTON"||(t.tagName==="A"&&t.className&&(String(t.className).indexOf("btn")>-1||String(t.className).indexOf("button")>-1||String(t.className).indexOf("cta")>-1));if(isBtn)continue;var cl=lum(prgb(getComputedStyle(t).color));if(cl<0)continue;var bgL=bgBehind(t);if(bgL<0)bgL=bgLumOf(t);if(bgL<0)continue;if(Math.abs(cl-bgL)<0.3){t.style.setProperty("color",bgL>0.5?"#111":"#fff","important");}}}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",fix);}else{fix();}setTimeout(fix,300);setTimeout(fix,900);setTimeout(fix,1800);window.addEventListener("scroll",function(){fix();},{passive:true});window.addEventListener("resize",function(){fix();},{passive:true});})();</scr'+'ipt>';
...
const PRODUCT_MODAL_SCRIPT = '<scr'+'ipt id="_wsProductModal">(function(){function txt(el){return el?(el.textContent||""):"";}function open(card){var ov=document.createElement("div");ov.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center;z-index:99999;padding:20px";var box=document.createElement("div");box.style.cssText="background:#fff;color:#111;max-width:520px;width:100%;border-radius:16px;overflow:auto;max-height:90vh;box-shadow:0 24px 70px rgba(0,0,0,.45);position:relative";var imgEl=card.querySelector(".wsp-img");if(imgEl){var im=document.createElement("div");im.style.cssText="width:100%;aspect-ratio:4/3;background-size:cover;background-position:center;background-image:"+getComputedStyle(imgEl).backgroundImage;box.appendChild(im);}var body=document.createElement("div");body.style.cssText="padding:24px";var nm=document.createElement("div");nm.style.cssText="font-size:1.4rem;font-weight:800;margin-bottom:6px";nm.textContent=txt(card.querySelector(".wsp-name"));body.appendChild(nm);var pr=document.createElement("div");pr.style.cssText="font-size:1.15rem;font-weight:700;margin-bottom:14px";pr.textContent=txt(card.querySelector(".wsp-price"));body.appendChild(pr);var ds=document.createElement("div");ds.style.cssText="opacity:.75;line-height:1.6;margin-bottom:18px";ds.textContent=txt(card.querySelector(".wsp-desc"));body.appendChild(ds);var buy=card.querySelector(".wsp-buy");if(buy){var cb=buy.cloneNode(true);cb.style.marginTop="4px";cb.addEventListener("click",function(e){if(e&&e.preventDefault)e.preventDefault();ov.remove();try{buy.click();}catch(err){}});body.appendChild(cb);}box.appendChild(body);var cl=document.createElement("button");cl.textContent="×";cl.setAttribute("aria-label","Close");cl.style.cssText="position:absolute;top:10px;right:12px;background:rgba(0,0,0,.06);border:none;border-radius:50%;width:34px;height:34px;font-size:20px;line-height:1;cursor:pointer;color:#111";cl.onclick=function(){ov.remove();};box.appendChild(cl);ov.appendChild(box);ov.addEventListener("click",function(e){if(e.target===ov)ov.remove();});function onEsc(e){if(e.key==="Escape"){ov.remove();document.removeEventListener("keydown",onEsc);}}document.addEventListener("keydown",onEsc);document.body.appendChild(ov);}function attach(){var cards=document.querySelectorAll(".wsp-card");for(var i=0;i<cards.length;i++){var c=cards[i];if(c.getAttribute("data-wspm"))continue;c.setAttribute("data-wspm","1");c.style.cursor="pointer";c.addEventListener("click",function(e){var t=e.target;while(t&&t!==this){if(t.className&&String(t.className).indexOf("wsp-buy")>-1)return;t=t.parentNode;}open(e.currentTarget);});}}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",attach);}else{attach();}setTimeout(attach,600);setTimeout(attach,1500);})();</scr'+'ipt>';
const CATEGORY_TABS_SCRIPT = '<scr'+'ipt id="_wsCategoryTabs">(function(){function move(ind,tab){if(!ind||!tab)return;ind.style.left=tab.offsetLeft+"px";ind.style.width=tab.offsetWidth+"px";}function init(tabs){if(tabs.getAttribute("data-wsct"))return;tabs.setAttribute("data-wsct","1");var ind=tabs.querySelector(".wsp-tab-ind");var btns=tabs.querySelectorAll(".wsp-tab");var grid=(tabs.parentNode&&tabs.parentNode.querySelector(".wsp-grid"))||document.querySelector(".wsp-grid");function activate(btn){for(var i=0;i<btns.length;i++)btns[i].classList.remove("wsp-tab-on");btn.classList.add("wsp-tab-on");move(ind,btn);var cat=btn.getAttribute("data-cat")||"";if(grid){var cards=grid.querySelectorAll(".wsp-card");for(var j=0;j<cards.length;j++){var cc=cards[j].getAttribute("data-category")||"";cards[j].style.display=(!cat||cc===cat)?"":"none";}}}for(var b=0;b<btns.length;b++){btns[b].addEventListener("click",function(){activate(this);});}var on=tabs.querySelector(".wsp-tab-on")||btns[0];if(on)move(ind,on);}function run(){var t=document.querySelectorAll(".wsp-tabs");for(var i=0;i<t.length;i++)init(t[i]);}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",run);}else{run();}setTimeout(run,400);setTimeout(run,1200);window.addEventListener("resize",function(){var t=document.querySelectorAll(".wsp-tabs");for(var i=0;i<t.length;i++){var ind=t[i].querySelector(".wsp-tab-ind");var on=t[i].querySelector(".wsp-tab-on");if(ind&&on){ind.style.left=on.offsetLeft+"px";ind.style.width=on.offsetWidth+"px";}}});})();</scr'+'ipt>';
const HEADER_FIX_SCRIPT = '<scr'+'ipt id="_wsHeaderFix">(function(){try{var s=document.createElement("style");s.textContent="header>nav,header .site-nav,header>.container{display:flex!important;align-items:center!important;flex-wrap:wrap!important;gap:14px 26px!important;width:100%}"+"@media(min-width:900px){header>nav,header .site-nav,header>.container{justify-content:space-between!important;flex-wrap:nowrap!important}}"+"header>nav>*,header .site-nav>*{flex:0 1 auto;min-width:0}"+"header>nav ul,header .site-nav ul{display:flex!important;align-items:center;flex-wrap:wrap;gap:12px 22px!important;list-style:none;margin:0;padding:0}";document.head.appendChild(s);}catch(e){}})();</scr'+'ipt>';
const CSS_STRIP_SCRIPT = '<scr'+'ipt id="_wsCssStrip">(function(){function count(s,ch){var n=0,i=0;while((i=s.indexOf(ch,i))>-1){n++;i++;}return n;}function looksLikeCss(t){if(!t||t.length<40)return false;var open=count(t,"{"),close=count(t,"}");if(open<2||close<2)return false;var semi=count(t,";");if(t.indexOf("*/")>-1||t.indexOf("@media")>-1||t.indexOf("@keyframes")>-1)return true;return t.indexOf(":")>-1&&semi>=3;}function inCode(el){while(el){var g=el.tagName;if(g==="STYLE"||g==="SCRIPT"||g==="PRE"||g==="CODE"||g==="TEXTAREA")return true;el=el.parentElement;}return false;}function strip(){try{if(!document.body)return;var w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,null);var kill=[],node;while(node=w.nextNode()){var t=node.nodeValue||"";if(!looksLikeCss(t))continue;if(inCode(node.parentElement))continue;kill.push(node);}for(var i=0;i<kill.length;i++){var n=kill[i];var par=n.parentElement;if(n.parentNode)n.parentNode.removeChild(n);if(par&&par!==document.body&&(par.textContent||"").trim()===""&&par.children.length===0&&par.parentNode)par.parentNode.removeChild(par);}}catch(e){}}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",strip);}else{strip();}setTimeout(strip,300);setTimeout(strip,1000);})();</scr'+'ipt>';
function withReveal(html){
  if(!html||html.indexOf('_wsReveal')>-1)return html;
  var bi=html.lastIndexOf('</body>');
  var _both = NAV_CONTRAST_SCRIPT + CTA_EDGE_SCRIPT + NAV_DEDUPE_SCRIPT + TEXT_CONTRAST_SCRIPT + PRODUCT_MODAL_SCRIPT + CATEGORY_TABS_SCRIPT + HEADER_FIX_SCRIPT + CSS_STRIP_SCRIPT + REVEAL_SCRIPT;
...
If you create any image-like placeholder area, it MUST use this format. Never create a styled box that looks like an image area without ws-img-slot class and data-slot attribute. The header must be ONE clean horizontal row on desktop: brand/logo on the left, nav links, and any CTA button on the right, all vertically centered with even spacing - it must NEVER overlap or cram into a messy second row. ALL css must live inside a single complete style block; NEVER let css rules, selectors, or comments (like /* */) appear as visible text on the page or outside a style tag. NEVER stack, layer, or overlap multiple ws-img-slots in the same spot - each image area gets EXACTLY ONE ws-img-slot. If you want a layered or stacked-cards look, use ONE ws-img-slot plus decorative non-uploadable shapes behind it (plain divs with no ws-img-slot class), never two or three upload slots piled on top of each other - overlapping upload slots collide in the editor and are a bug.
...
<meta name="ws-build" content="2026-06-10-r269">
<script>window._wsBuild="2026-06-10-r269";console.log("%c[Websprout] build 2026-06-10-r269 — nav contrast now measures each header element background individually (per-element)","color:#4ade80;font-weight:700")</script>
...
function setPreview(html){
  ...
  function wsRenderProducts(list){var esc=function(x){x=String(x==null?"":x);return x.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;").split(String.fromCharCode(34)).join("&quot;");};var css='<style>.wsp-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:22px;max-width:1100px;margin:0 auto;padding:8px 0}.wsp-card{border:1px solid rgba(128,128,128,.18);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;background:rgba(128,128,128,.04)}.wsp-img{aspect-ratio:4/3;background-size:cover;background-position:center}.wsp-noimg{background:linear-gradient(135deg,rgba(128,128,128,.12),rgba(128,128,128,.05))}.wsp-body{padding:16px;display:flex;flex-direction:column;gap:6px}.wsp-name{font-weight:700;font-size:1.05rem}.wsp-price{font-weight:700;opacity:.85}.wsp-desc{font-size:.9rem;opacity:.7;line-height:1.5}.wsp-buy{margin-top:8px;display:inline-block;text-align:center;padding:10px 16px;border:1px solid currentColor;border-radius:10px;text-decoration:none;color:inherit;font-weight:600}.wsp-tabs{position:relative;display:flex;flex-wrap:wrap;gap:6px;justify-content:center;max-width:1100px;margin:0 auto 26px;padding-bottom:12px}.wsp-tab{background:none;border:none;font:inherit;font-weight:700;font-size:.9rem;letter-spacing:.03em;text-transform:uppercase;color:inherit;opacity:.45;cursor:pointer;padding:6px 14px;transition:opacity .2s}.wsp-tab.wsp-tab-on{opacity:1}.wsp-tab-ind{position:absolute;bottom:0;height:2px;background:currentColor;border-radius:2px;transition:left .28s ease,width .28s ease;left:0;width:0}</style>';if(!list||!list.length){return css+'<div style="text-align:center;opacity:.55;padding:44px 0;font-size:1rem">Your products show here - add them from the Manage menu (Products &amp; cart).</div>';}var cards="";for(var i=0;i<list.length;i++){var p=list[i]||{};var img=p.img?('<div class="wsp-img" style="background-image:url('+esc(p.img)+')"></div>'):'<div class="wsp-img wsp-noimg"></div>';var price=p.price?('<div class="wsp-price">'+esc(p.price)+'</div>'):"";var desc=p.desc?('<div class="wsp-desc">'+esc(p.desc)+'</div>'):"";var pu=String(p.payUrl||"");var buy=(pu.indexOf("https://")===0)?('<a class="wsp-buy" href="'+esc(pu)+'" target="_blank" rel="noopener">Buy now</a>'):('<button class="wsp-buy" type="button" onclick="if(window.wsAddToCart)wsAddToCart('+i+')">Add to cart</button>');cards+='<div class="wsp-card" data-category="'+esc(p.category||"")+'">'+img+'<div class="wsp-body"><div class="wsp-name">'+esc(p.name||"Product")+'</div>'+price+desc+buy+'</div></div>';}var _cats=[];for(var _ci=0;_ci<list.length;_ci++){var _cc=String((list[_ci]||{}).category||"").trim();if(_cc&&_cats.indexOf(_cc)<0)_cats.push(_cc);}var _tabs="";if(_cats.length>=2){_tabs='<div class="wsp-tabs"><button type="button" class="wsp-tab wsp-tab-on" data-cat="">All</button>';for(var _ti=0;_ti<_cats.length;_ti++){_tabs+='<button type="button" class="wsp-tab" data-cat="'+esc(_cats[_ti])+'">'+esc(_cats[_ti])+'</button>';}_tabs+='<span class="wsp-tab-ind"></span></div>';}return css+_tabs+'<div class="wsp-grid">'+cards+'</div>';}
  ...
  try{ if(window._wsCartHtml){ var _cbi=out.lastIndexOf('</body>'); out=_cbi>-1?(out.slice(0,_cbi)+window._wsCartHtml+out.slice(_cbi)):(out+window._wsCartHtml); } }catch(e){}
...
(function(){
  ...
  function makeRow(p){ p=p||{}; var row=document.createElement("div"); row.className="wsProdRow"; row.style.cssText="display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:12px;border:1px solid rgba(255,255,255,.1);border-radius:12px;margin-bottom:10px;background:rgba(255,255,255,.02)"; fld(row,"Product name",p.name,"name"); var _prF=fld(row,"Price (e.g. $24)",p.price,"price"); _prF.addEventListener("input",function(){ var d=_prF.value.replace(/[^0-9.]/g,""); _prF.value=d?("$"+d):""; }); _prF.addEventListener("blur",function(){ var d=_prF.value.replace(/[^0-9.]/g,""); if(d!==""){ var n=parseFloat(d); _prF.value="$"+((isFinite(n))?((n%1===0)?String(n):n.toFixed(2)):d); } }); var _imgF=fld(row,"Image URL, or use Upload below",p.img,"img",true); var _upB=document.createElement("button"); _upB.type="button"; _upB.textContent="📷 Upload photo"; _upB.style.cssText="grid-column:1/-1;justify-self:start;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);border-radius:8px;color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;padding:7px 12px;margin:-2px 0 2px"; var _upI=document.createElement("input"); _upI.type="file"; _upI.accept="image/*"; _upI.style.display="none"; _upB.onclick=function(){ _upI.click(); }; _upI.onchange=function(){ var f=_upI.files&&_upI.files[0]; if(!f)return; if(typeof compressImage!=="function"){ if(typeof toast==="function")toast("Photo upload unavailable here",3000); return; } _upB.textContent="Uploading..."; try{ compressImage(f,function(u){ _imgF.value=u; _upB.textContent="✓ Photo added"; setTimeout(function(){ _upB.textContent="📷 Change photo"; },1500); }); }catch(e){ _upB.textContent="📷 Upload photo"; } }; row.appendChild(_upB); row.appendChild(_upI); fld(row,"Short description",p.desc,"desc",true); fld(row,"Category, e.g. Tees (optional)",p.category,"category",true); fld(row,"Stripe payment link (optional)",p.payUrl,"payUrl",true); var del=document.createElement("button"); del.type="button"; del.textContent="Remove"; del.style.cssText="grid-column:1/-1;justify-self:start;background:transparent;border:none;color:#ff9b9b;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;padding:2px 0"; del.onclick=function(){ row.remove(); }; row.appendChild(del); return row; }
  ...
  function doSave(){
    ...
    .then(function(j){
      ...
      try{ if(j&&j.products) window._wsProducts=j.products; fetch("/store-preview?siteId="+encodeURIComponent(s)+"&key="+encodeURIComponent(k)).then(function(r){return r.text();}).then(function(t){ window._wsCartHtml=t||""; if(typeof setPreview==="function") setPreview(gHTML); }).catch(function(){ if(typeof setPreview==="function") setPreview(gHTML); }); }catch(e){ if(typeof setPreview==="function") setPreview(gHTML); }
    })
    .catch(function(){ tt("Could not save products"); });
  }
  function openProducts(){
    buildModal();
    var s=sid(), k=skey(); var w=$("wsProdRows"); if(w) w.innerHTML=""; if(s){ try{ fetch("/store-preview?siteId="+encodeURIComponent(s)+"&key="+encodeURIComponent(k)).then(function(r){return r.text();}).then(function(t){ window._wsCartHtml=t||""; try{ if(typeof gHTML!=="undefined"&&typeof setPreview==="function") setPreview(gHTML); }catch(e){} }).catch(function(){}); }catch(e){} }
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/store-preview') return doStorePreview(request, env);
...
const BUILD_ID = '2026-06-10-r269';
...
function planPages(prompt, type){
  var p = (prompt||'').toLowerCase();
  if (type==='store' || /\b(shop|store|sell|selling|product|products|ecommerce|e-commerce|merch|boutique|online store|catalog|checkout|buy online|apparel|clothing|clothes|[a-z]+wear|fashion|jewelry|jewellery|footwear|sneakers|threads)\b/.test(p)) {
    return [
...
function cartInjection(products, siteId, preview){
...
(function(){
  try{
    ...
    var P=${dataJson};
    var PREVIEW=${preview?'true':'false'}; var KEY="wscart_"+SITE;
    ...
    function checkout(){ if(PREVIEW){ alert("Publish your site to accept payments. Checkout is disabled in preview."); return; }
...
function renderProductGrid(products, siteId, checkoutEnabled){
  ...
  var cards = products.map(function(p, i){
    ...
    return '<div class="wsp-card" data-category="' + esc(p.category||'') + '">' + img + '<div class="wsp-body"><div class="wsp-name">' + esc(p.name||'Product') + '</div>' + price + desc + buy + '</div></div>';
  }).join('');
  var css = '<style>.wsp-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:22px;max-width:1100px;margin:0 auto;padding:8px 0}.wsp-card{border:1px solid rgba(128,128,128,.18);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;background:rgba(128,128,128,.04)}.wsp-img{aspect-ratio:4/3;background-size:cover;background-position:center}.wsp-noimg{background:linear-gradient(135deg,rgba(128,128,128,.12),rgba(128,128,128,.05))}.wsp-body{padding:16px;display:flex;flex-direction:column;gap:6px}.wsp-name{font-weight:700;font-size:1.05rem}.wsp-price{font-weight:700;opacity:.85}.wsp-desc{font-size:.9rem;opacity:.7;line-height:1.5}.wsp-buy{margin-top:8px;display:inline-block;text-align:center;padding:10px 16px;border:1px solid currentColor;border-radius:10px;text-decoration:none;color:inherit;font-weight:600}.wsp-buy:hover{opacity:.8}.wsp-tabs{position:relative;display:flex;flex-wrap:wrap;gap:6px;justify-content:center;max-width:1100px;margin:0 auto 26px;padding-bottom:12px}.wsp-tab{background:none;border:none;font:inherit;font-weight:700;font-size:.9rem;letter-spacing:.03em;text-transform:uppercase;color:inherit;opacity:.45;cursor:pointer;padding:6px 14px;transition:opacity .2s}.wsp-tab.wsp-tab-on{opacity:1}.wsp-tab-ind{position:absolute;bottom:0;height:2px;background:currentColor;border-radius:2px;transition:left .28s ease,width .28s ease;left:0;width:0}</style>';
  var _cats=[]; products.forEach(function(pp){ var cc=String(pp.category||'').trim(); if(cc && _cats.indexOf(cc)<0) _cats.push(cc); }); var _tabs=''; if(_cats.length>=2){ _tabs='<div class="wsp-tabs"><button type="button" class="wsp-tab wsp-tab-on" data-cat="">All</button>'; _cats.forEach(function(cc){ _tabs+='<button type="button" class="wsp-tab" data-cat="'+esc(cc)+'">'+esc(cc)+'</button>'; }); _tabs+='<span class="wsp-tab-ind"></span></div>'; } return css + _tabs + '<div class="wsp-grid">' + cards + '</div>';
}
async function doProducts(request, env){
  try {
    ...
    products = products.map(function(p){ var pu=String(p.payUrl||'').trim().slice(0,600); if(!/^https:\/\//i.test(pu)) pu=''; return { name:String(p.name||'').slice(0,140), price:String(p.price||'').slice(0,40), img:String(p.img||'').slice(0,600), desc:String(p.desc||'').slice(0,500), payUrl:pu, category:String(p.category||'').slice(0,40) }; }).filter(function(p){ return p.name; });
    await env.KV.put('products:' + b.siteId, JSON.stringify(products));
    return succeed({ ok:true, products: products });
  } catch(e){ return fail(e.message); }
}
async function doStorePreview(request, env){
  try {
    if (!env.KV) return new Response("", {status:200, headers:{"Content-Type":"text/html"}});
    const url = new URL(request.url);
    const siteId = url.searchParams.get("siteId") || "";
    const key = url.searchParams.get("key") || "";
    if (!siteId || key !== (await siteKey(siteId, env))) return new Response("", {status:403});
    let products = [];
    try { const raw = await env.KV.get("products:" + siteId); if (raw) products = JSON.parse(raw); } catch(e){}
    const cart = (products && products.length) ? cartInjection(products, siteId, true) : "";
    return new Response(cart, {status:200, headers:{"Content-Type":"text/html; charset=utf-8"}});
  } catch(e){ return new Response("", {status:200, headers:{"Content-Type":"text/html"}}); }
}
...
async function doGenerate(request, env) {
  ...
  try {
    ...
    try {
      var _isShopGen = /\b(shop|store|sell|selling|product|products|ecommerce|e-commerce|merch|boutique|online store|catalog|checkout|buy online|apparel|clothing|clothes|[a-z]+wear|fashion|jewelry|jewellery|footwear|sneakers|threads)\b/i.test(prompt||'');
...
async function doModify(request, env) {
  ...
  try {
    ...
    var _storeNote = '';
    try {
      var _shopBuild = /\b(shop|store|storefront|sell|selling|product|products|purchase|checkout|cart|for sale|on sale|merch|catalog|apparel|clothing|streetwear)\b/i.test(body.instruction||'');
      if (_shopBuild && cleaned.indexOf('<!--WS_PRODUCTS-->') === -1) {
        var _shopSec = '\n<section id="shop" style="padding:64px 24px"><div style="max-width:1100px;margin:0 auto"><h2 style="text-align:center;margin:0 0 28px;font-size:2rem">Shop</h2>\n<!--WS_PRODUCTS-->\n</div></section>\n';
        var _bi = cleaned.lastIndexOf('</body>');
        cleaned = _bi > -1 ? (cleaned.slice(0,_bi) + _shopSec + cleaned.slice(_bi)) : (cleaned + _shopSec);
        _storeNote = ' I added a connected store section \u2014 add your products in Manage \u2192 Products & cart and they will show there with working Buy / Add-to-cart buttons.';
      }
    } catch(e){}
    return succeed({ html: withReviews(withForms(withFix(withReveal(cleaned)), mSite), mSite), message: 'Done! Your site has been updated.' + _storeNote });
  } catch(e) { return fail(e.message); }
}
